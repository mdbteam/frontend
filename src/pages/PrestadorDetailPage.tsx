...
const citaSchema = z.object({
  detalles: z
    .string()
    .min(10, "Por favor, da más detalles del trabajo (mín. 10 caracteres)."),
  duracion_min: z
    .number()
    .min(30, "La duración mínima es 30 min.")
    .optional(),
});
...
const fetchPrestadorProfile = async (id: string) =>
  (await apiProveedores.get<PrestadorPublicoDetalle>(`/prestadores/${id}`))
    .data;

const fetchPrestadorDisponibilidad = async (id: string) =>
  (await apiCalendario.get<BloquePublico[]>(`/prestadores/${id}/disponibilidad`))
    .data;

const fetchMyCitas = async (token: string | null) => {
  if (!token) throw new Error("No estás autenticado");
  return (
    await apiCalendario.get<CitaDetail[]>("/citas/me", {
...
const createCita = async ({
  ...
  return apiCalendario.post('/citas', payload, {
    headers: { Authorization: `Bearer ${token}` },
  });
};
...
export default function PrestadorDetailPage() {
  ...
  const [selectionToBook, setSelectionToBook] = useState<DateSelectArg | null>(
    null
  );
  ...
  const { data: profile, isLoading: isLoadingProfile, error: errorProfile } =
    useQuery({
  ...
  const createCitaMutation = useMutation({
    ...
    onError: () =>
      setModalInfo({
        isOpen: true,
        title: 'Error',
        description: 'No se pudo agendar.',
        type: 'error',
      }),
  });

  const esMiPerfil =
    !!user && profile?.id_usuario.toString() === user.id;

  useEffect(() => {
    if (esMiPerfil) navigate('/calendario', { replace: true });
  }, [esMiPerfil, navigate]);
  ...
  const dataForCalendar = useMemo((): EventInput[] => {
    ...
    const misEventos = (misCitas || [])
      ...
      .map((cita) => ({
        ...
        start: cita.fecha_hora_cita,
          ...
          new Date(cita.fecha_hora_cita).getTime() + 60 * 60000
        ).toISOString(),
    ...
    const misHorariosOcupados = new Set(
      misEventos.map((e) =>
        new Date(e.start as string).toISOString()
      )
    );
    ...
    const eventosDisponibilidad = (disponibilidad || [])
      ...
      .map((slot, index) => {
        const esDisponible =
          slot.estado.toLowerCase() === 'disponible';
        return {
          id: `slot-${index}`,
  ...
  // 🔥🔥🔥 FUNCIÓN CORREGIDA + VALIDACIÓN DE FECHAS PASADAS
  const handleDateSelect = (selectInfo: DateSelectArg) => {
    ...
    if (!isAuthenticated) {
      setModalInfo({
        ...
        description: 'Inicia sesión para agendar.',
        type: 'info',
      });
      return;
    }

    // 🚫⏳ VALIDACIÓN NUEVA: evitar agendar en el pasado
    const ahora = new Date();
    if (selectInfo.start < ahora) {
      setModalInfo({
        isOpen: true,
        title: 'Fecha inválida',
        description: 'No es posible agendar en una fecha u hora pasada.',
        type: 'error',
      });
      return;
    }
    ...
    const bloqueDisponible = allEvents.find(
      (e) =>
        e.extendedProps.tipo === 'slot' &&
        e.extendedProps.estado === 'disponible' &&
        e.start &&
        e.end &&
    ...
    const conflicto = allEvents.find(
      (e) =>
        e.extendedProps.tipo !== 'slot' &&
        e.start &&
        e.end &&
        selectInfo.start < e.end &&
        selectInfo.end > e.start
    );
    ...
    if (!bloqueDisponible || conflicto) {
      setModalInfo({
        ...
        description: 'Selecciona un bloque verde libre.',
        type: 'error',
      });
      return;
    }

    // CALCULAR DURACIÓN REAL DEL DRAG
    const diffMin =
      (selectInfo.end.getTime() -
        selectInfo.start.getTime()) /
      60000;

    const finalDuration = diffMin < 30 ? 30 : diffMin;

    reset({
      detalles: "",
      duracion_min: finalDuration,
    });
  ...
  const onCitaSubmit: SubmitHandler<CitaFormInputs> = (
    data
  ) => {
  ...
  const isLoading =
    isLoadingProfile ||
    isLoadingDisp ||
    (isAuthenticated && isLoadingCitas) ||
    esMiPerfil;
          ...
          <div className="md:col-span-1 space-y-6">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 text-center shadow-lg">
              {profile.foto_url ? (
                <img
                  ...
                  alt={`Foto de ${profile.nombres}`}
                  className="h-32 w-32 rounded-full border-4 border-slate-700 mx-auto object-cover"
                />
              ) : (
              ...
              <div className="flex justify-center gap-6 mt-6 pt-4 border-t border-slate-800">
                <div className="text-center">
                  <span className="text-xl font-bold text-white flex justify-center gap-1">
                    <FaStar className="text-yellow-400" />{' '}
                    {profile.puntuacion_promedio.toFixed(1)}
                  </span>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 space-y-4 shadow-md">
              ...
              <p className="text-sm text-slate-300 leading-relaxed">
                {profile.perfil?.biografia || 'Sin biografía.'}
              </p>

              <h3 className="text-xs font-bold text-slate-500 uppercase mt-4">
                Especialidades
              </h3>

              <div className="flex flex-wrap gap-2">
                {(profile.oficios || []).map((c) => (
                  <span
                    key={c}
                    className="bg-slate-800 text-cyan-300 text-xs px-2 py-1 rounded border border-slate-700"
                  >
                    {c}
                  </span>
                ))}
                  ...
                  {profile.portafolio.map((imgUrl, idx) => (
                    ...
                    >
                      <img
                        src={imgUrl}
                        alt={`Trabajo ${idx + 1}`}
                        className="object-cover w-full h-24 hover:opacity-80 transition-opacity"
                      />
                    </button>
                  ))}
                </div>
            ...
            <div>
              <div className="mb-4 border-b border-slate-800 pb-4">
                ...
                <p className="text-slate-400 text-sm mt-1">
                  Haz clic o arrastra sobre los bloques{' '}
                  <span className="text-emerald-400 font-bold">verdes</span>{' '}
            ...
            <DialogHeader>
              ...
              <DialogDescription>
                ...
                <span className="text-white font-bold">
                  {selectionToBook?.start.toLocaleString('es-CL')}
                </span>
              </DialogDescription>
              ...
              <DialogDescription className="mt-2">
                ...
                <span className="text-emerald-400 font-bold">
                  {watch("duracion_min")} min
                </span>
              </DialogDescription>
            </DialogHeader>

            <form
              onSubmit={handleSubmit(onCitaSubmit)}
            ...
            >
              ...
              <div>
                ...
                <input
                  ...
                  step="30"
                  disabled
                  className="w-full bg-slate-950 border border-slate-700 text-white rounded p-2 mt-1 opacity-50 cursor-not-allowed"
